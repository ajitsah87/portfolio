          ...
          <div className="flex-1">
            <ul className="grid grid-cols-4 gap-6 ">
              {svg.map((src, i) => (
                ...
                onHoverEnd={()=> setDescrip('')}
                  key={i}
                  className="border-[1px]  hover:border-[#986dff] transition-colors duration-500 ease-linear border-neutral-800 rounded-[5px] p-8 bg-[#151515]"
                >
                  <img src={src.title} className="h-16 w-16   text-[#986dff]" />
                </motion.li>
              ))}
            </ul>
          </div>
